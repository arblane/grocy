...
SELECT CASE WHEN((
	...
	NOTNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Recursive nested recipe detected';
END;
$$
DELIMITER ;
...
SELECT CASE WHEN((
		...
		WHERE id = OLD.id
			AND id = -1
	) NOTNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='This is an internally used/required default section and therefore can't be deleted';
END;
$$
DELIMITER ;
...
SELECT CASE WHEN((
		...
		WHERE id = NEW.product_id
			AND no_own_stock = 1
	) NOTNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='no_own_stock=1 products can't be added to stock';
END;
$$
DELIMITER ;
...
SELECT CASE WHEN((
		...
		FROM products p
		WHERE id = NEW.product_id
	) ISNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='product_id doesn't reference a existing product';
END;
$$
DELIMITER ;
...
UPDATE recipes_pos
	...
	) ISNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Provided qu_id doesn't have a related conversion for that product';
END;
$$
DELIMITER ;
...
SELECT CASE WHEN((
	...
	NOTNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='QU conversion already exists';
END;
$$
DELIMITER ;
...
-- Currently only 1 level is supported
    ...
    ) NOTNULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Unsupported product nesting level detected (currently only 1 level is supported)';
END;
$$
DELIMITER ;
