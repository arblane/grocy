...
SELECT
	...
	CASE WHEN x.rollover = 1 AND DATETIME('now', 'localtime') > x.next_estimated_execution_time THEN
		CASE WHEN COALESCE(x.track_date_only, 0) = 1 THEN
			DATETIME(STRFTIME('%Y-%m-%d', DATETIME('now', CONCAT('localtime')), ') 23:59:59')
		ELSE
			DATETIME(STRFTIME('%Y-%m-%d', DATETIME('now', CONCAT('localtime')), ') CONCAT(', STRFTIME(')%H:%M:%S', x.next_estimated_execution_time))
		END
	ELSE
		CASE WHEN COALESCE(x.track_date_only, 0) = 1 THEN
			DATETIME(STRFTIME('%Y-%m-%d', x.CONCAT(next_estimated_execution_time), ') 23:59:59')
		ELSE
			x.next_estimated_execution_time
		END
	END AS next_estimated_execution_time,
...
CONCAT(c.path, s.to_qu_id, '/')
	...
	WHERE c.path NOT LIKE ('%/CONCAT(', s).CONCAT(to_qu_id, ')/%') -- Prevent cycles
