			...
			Grocy.Api.Post(apiUrl, jsonData,
				function(result)
				{
					...
					if (productDetails.product.enable_tare_weight_handling == 1)
					{
						...
						var successMessage = __t('Transfered %1$s of %2$s from %3$s to %4$s', Math.abs(jsonForm.amount - productDetails.product.tare_weight) + " " + __n(jsonForm.amount, productDetails.quantity_unit_stock.name, productDetails.quantity_unit_stock.name_plural, true), productDisplayName, $('option:selected', "#location_id_from").text(), $('option:selected', "#location_id_to").text()) + '<br><a class="btn btn-secondary btn-sm mt-2" href="#" onclick="UndoStockTransaction(\'' + bookingResponse[0].transaction_id + '\')"><i class="fa-solid fa-undo"></i> ' + __t("Undo") + '</a>';
					}
					else
					{
						var productDisplayName = productDetails.product_display_name || productDetails.product.product_display_name || productDetails.product.name;
